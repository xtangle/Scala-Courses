...
package object barneshut {
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad =
      if (size > minimumSize) {
        ...
        val qs: Float = size / 4
        val (wx, ex, ny, sy) =
          (centerX - qs, centerX + qs, centerY - qs, centerY + qs)
        val fork = Fork(Empty(wx, ny, hs), Empty(ex, ny, hs),
          Empty(wx, sy, hs), Empty(ex, sy, hs))
        bodies.foldLeft(fork)((f, body) => f.insert(body)).insert(b)
      } else {
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      def bounded(x: Int, min: Int, max: Int): Int =
        if (x < min) min else if (x > max) max else x
      val ix: Int = bounded(((b.x - boundaries.minX) * sectorPrecision / boundaries.width).toInt, 0, sectorPrecision - 1)
      val iy: Int = bounded(((b.y - boundaries.minY) * sectorPrecision / boundaries.height).toInt, 0, sectorPrecision - 1)
      matrix(iy * sectorPrecision + ix) += b
      this
    }
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      matrix.zip(that.matrix).zipWithIndex
        .foreach{ case ((cb1, cb2), idx) => matrix(idx) = cb1.combine(cb2) }
      this
    }
