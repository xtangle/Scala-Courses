...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    means.map(m => (m, GenSeq())).toMap ++ points.groupBy(findClosest(_, means))
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zip(newMeans).forall{ case (p1, p2) => squaredDistance(p1, p2) <= eta }
  }

  private def squaredDistance(p1: Point, p2: Point): Double = {
    val dx = p1.x - p2.x
    val dy = p1.y - p2.y
    val dz = p1.z - p2.z
    dx*dx + dy*dy + dz*dz
