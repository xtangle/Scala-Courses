...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.aggregate(0)((count, wa) => {
      if (wa.mentionsLanguage(lang)) count + 1 else count
    }, Integer.sum)
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd.flatMap(wa => wa.text.split(' ').filter(langs.contains).distinct.map((_, wa))).groupByKey
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd.flatMap(wa => wa.text.split(' ')
      .filter(langs.contains).distinct.map((_, 1)))
  ...
  def main(args: Array[String]) {
    ...
    println(Console.GREEN + timing + Console.WHITE)
    sc.stop()
  }
