...
class StackOverflow extends Serializable {
  ...
  def scoredPostings(grouped: RDD[(Int, Iterable[(Posting, Posting)])]): RDD[(Posting, Int)] = {
    ...
    grouped.values.map(_.unzip).map{case (qs, as) => (qs.head, answerHighScore(as.toArray))}
  }
  ...
  def vectorPostings(scored: RDD[(Posting, Int)]): RDD[(Int, Int)] = {
    ...
    // Important: cache the results because the resulting RDD will be used very often
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    val newMeansArr = vectors.map(p => (findClosest(p, means), p))
      .groupByKey.mapValues(averageVectors).collect

    newMeansArr.foreach{case (i, m) => newMeans.update(i, m)}
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      // index of most common language and how many times it appeared
      val mostLang = vs.groupBy(_._1).map(v => (v._1 / langSpread, v._2.size)).maxBy(_._2)

      // most common language in the cluster
      val langLabel: String   = langs(mostLang._1)

      // percent of the questions in the most common language
      val langPercent: Double = mostLang._2 * 100d / vs.size

      val clusterSize: Int    = vs.size
      val medianScore: Int    = {
        if (clusterSize == 0) 0
        else {
          val sorted = vs.unzip._2.toArray.sorted
          if (clusterSize % 2 == 1)
            sorted((clusterSize - 1) / 2)
          else {
            val m = clusterSize / 2
