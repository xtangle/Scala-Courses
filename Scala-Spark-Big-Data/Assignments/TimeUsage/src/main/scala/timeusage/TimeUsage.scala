...
object TimeUsage {
  ...
  def row(line: List[String]): Row =
    Row(line.head.trim :: line.tail.map(_.toDouble) : _*)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    @tailrec
    def classifiedColumnsRec(columnNames: List[String],
                             primary: ListBuffer[Column] = new ListBuffer[Column],
                             working: ListBuffer[Column] = new ListBuffer[Column],
                             other: ListBuffer[Column] = new ListBuffer[Column]): (List[Column], List[Column], List[Column]) = {
      columnNames match {
        case Nil => (primary.toList, working.toList, other.toList)
        case c::cs =>
          val (pref3, pref5) = (Try(c.take(3)).getOrElse(""), Try(c.take(5)).getOrElse(""))
          val newcol = new Column(c)
          if (Set("t01", "t03", "t11").contains(pref3) || Set("t1801", "t1803").contains(pref5))
            primary += newcol
          else if (pref3 == "t05" || pref5 == "t1805")
            working += newcol
          else if (Set("t02", "t04", "t06", "t07", "t08", "t09",
            "t10", "t12", "t13", "t14", "t15", "t16", "t18").contains(pref3))
            other += newcol
          classifiedColumnsRec(cs, primary, working, other)
      }
    }

    classifiedColumnsRec(columnNames)
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column =
      when(df("telfs").between(1, 2), "working").otherwise("not working").as("working")
