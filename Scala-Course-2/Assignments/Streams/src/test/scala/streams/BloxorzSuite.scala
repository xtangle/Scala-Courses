...
class BloxorzSuite extends FunSuite {
  ...
  test("solution for level 0 should be empty") {
    new Level0 {
  ...
  test("neighbors level 1") {
    new Level1 {
      assert(startBlock.neighbors.toSet === Set(
  ...
  test("legalNeighbors level 1") {
    new Level1 {
      assert(startBlock.legalNeighbors.toSet === Set(
        (Block(Pos(1, 2), Pos(1, 3)), Right),
        (Block(Pos(2, 1), Pos(3, 1)), Down)
      ))
    }
  ...
  test("newNeighborsOnly level 1") {
    new Level1 {
      ...
      val expected: Stream[(Block, List[Move])] = Set(
        (Block(Pos(2,1),Pos(3,1)), List(Down,Left,Up))
      ).toStream
